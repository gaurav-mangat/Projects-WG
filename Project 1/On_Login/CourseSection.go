...
func UpdateProgress(username string) {
	...
	// Display all courses
	fmt.Println("\033[1;34mAvailable Courses:\033[0m")
	for _, course := range coursesData.Courses {
	...
	var courseID int
	fmt.Print("\033[1;34mEnter Course ID to update: \033[0m")
	fmt.Scan(&courseID)
	...
	fmt.Println("\033[1;34mLessons for Course ID", courseID, ":\033[0m")
	...
	for {
		...
		fmt.Print("\033[1;34mEnter Lesson ID to mark as completed (or type 0 to finish): \033[0m")
		fmt.Scan(&lessonID)
		...
		if !lessonFound {
			fmt.Println("\033[1;31mInvalid Lesson ID. No such lesson found for the given Course ID.\033[0m")
			continue
		}
		...
		if lessonAlreadyCompleted {
			fmt.Println("\033[1;31mLesson ID already completed. Please enter a different Lesson ID.\033[0m")
			continue
		}

		// Update progress
		userProgress.CourseProgress[courseID] = append(userProgress.CourseProgress[courseID], lessonID)
		fmt.Println("\033[1;32mLesson ID added successfully!\033[0m")

		// Update progress percentages
		updateProgressPercentages(userProgress, coursesData)
	}

	if err := utils.SaveProgress(progressfile); err != nil {
		fmt.Printf("\033[1;31mError saving progress: %v\033[0m\n", err)
		return
	}

	fmt.Println("\033[1;32mAll progress updates completed successfully!\033[0m")
}
...
func updateProgressPercentages(userProgress *models.Progress, coursesData models.CoursesData) {
	...
	if totalLessons > 0 {
		userProgress.OverallCompletion = float64(completedLessons) / float64(totalLessons) * 100
	} else {
...
func DisplayProgress(username string) {
	...
	fmt.Printf("Overall Completion: %.2f%%\n", userProgress.OverallCompletion)
	...
	for courseID, lessons := range userProgress.CourseProgress {
		...
		if completion, exists := userProgress.CourseCompletion[courseID]; exists {
			fmt.Printf("Completion for Course ID %d: %.2f%%\n", courseID, completion)
		}
	}
